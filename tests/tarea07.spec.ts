...
test.describe('Tarea 07 - Evidencias avanzadas', () => {
  test('Reto 1 - test.step()', async ({ page }) => {
    ...
    await test.step('Verificar inventario cargado', async () => {
      ...
      await page.screenshot({ path: './evidencias/tarea07/t01-inventario.png', fullPage: true });
    });
  });
  ...
  test('Reto 2 - testInfo.attach()', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('.inventory_container')).toBeVisible();
    await page.screenshot({ path: './evidencias/tarea07/t02-datos-login.png', fullPage: true });
  });
